...
function px_verifier_bulk_request() {	
	$users = get_users('role=participant');
	foreach($users as $user) {
		$id = $user->id;
		$item = get_the_author_meta('px_envato_item', $id);
		
		if($item == 'WordPress Blog Android App') {
			wp_update_user(array('ID' => $id, 'role' => 'px_wpba_customer'));
			
		}else if($item == 'WP Google Cloud Messaging') {
			wp_update_user(array('ID' => $id, 'role' => 'px_wpgcm_customer'));
...
add_action('admin_action_refactor_forum_roles', 'px_verifier_bulk_request');
